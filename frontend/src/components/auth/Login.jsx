...
const Login = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) { dispatch(setUser(res.data.user)); navigate("/"); toast.success(`Welcome back, ${res.data.user.fullname}!`); }
    ...
    <div className="min-h-screen bg-[#f8f7ff] flex">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <p className="text-gray-500 text-sm mb-8">Don't have an account? <Link to="/signup" className="text-purple-600 font-bold hover:underline">Create one free →</Link></p>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" name="password" value={input.password} onChange={change} required
                  className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white"
                  placeholder="••••••••" />
              </div>
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><Loader2 className="animate-spin" size={16} /> Signing in...</> : <><span>Sign In</span><ArrowRight size={16}/></>}
            </button>
          </form>
          <p className="text-center text-xs text-gray-400 mt-6">Demo recruiter: <strong>recruiter@seed.com</strong> / <strong>password123</strong></p>
        </div>
      </div>
    </div>
  );
